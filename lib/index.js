...
function generateQR (username, password) {
  ...
  const key = config.key.exportKey(config.enc.pubkey).toString(config.enc.string)
  const message = username + ' ' + pwdhash + ' ' + salt + ' ' + key + ' ' + ip + ' ' + nonce
  qr.generate(message, { small: true })
  establishPair(username, password, nonce)
}
...
function establishPair (username, password, nonce) {
  ...
  wss.on('connection', function connection (ws) {
    ws.on('message', function incoming (message) {
      try {
        let decipheredMessage = cryptojs.AES.decrypt(message, key.toString())
        decipheredMessage = decipheredMessage.toString(cryptojs.enc.Utf8).split(' ')
        const command = decipheredMessage[0]
        const deviceName = decipheredMessage[1]
        const deviceId = decipheredMessage[2]
        const deviceKey = JSON.parse(decipheredMessage[3])
