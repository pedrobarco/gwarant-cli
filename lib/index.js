...
function establishPair (username, password, nonce, iv) {
  const delimiter = '::'
  let newNonce = (parseInt(nonce, '16') + 1).toString(16)
    ...
    socket.on('data', data => {
      try {
        ...
        if (command === 'REGISTER' && deviceNonce === newNonce) {
          ...
          newNonce = (parseInt(nonce, '16') + 1).toString(16)
          let ack = 'ACK' + delimiter + newNonce
          // TODO: Sign ack
          // let sign = crypto.createSign('sha256')
          // sign.update(ack)
          // sign = sign.sign(config.key.exportKey('pkcs1'), 'base64')
          // let secret = ack + sign
          // secret = Buffer.from(secret, 'utf8')
          // ack = crypto.publicEncrypt(deviceKey, secret)
...
function connectPair () {
  ...
  udpserver.on('message', (msg, rinfo) => {
    ...
    // TODO: Check if ID exists in db, get associated username
    main.username = user

    appSocket = new net.Socket();
    appSocket.connect(8080, ip, function() {
      appSocket.on('error', function error (error) {
        //console.log('Cya in another life brother')
        //checkHeartbeat()
      })
	  try {
		  const timestamp = Math.floor(present())
...
function validateTimestamp (timestamp) {
  const currentTimestamp = Math.floor(present())
